<!-- app.js -->
/**
 * MANGAVERSE - Core SPA Script (Fixed CORS & Deep Link Routing)
 * Dark Minimalist Architecture with Bottom Navigation Controls.
 */

// Menggunakan CORS Proxy publik untuk menembus batasan keamanan browser jika API tidak menyediakan header CORS
const CORS_PROXY = 'https://cors-anywhere.herokuapp.com/'; // Atau proxy alternatif lainnya
...
// Helper untuk fetch data yang aman dari blokir CORS browser
async function safeFetch(url) {
    // Anda bisa mencoba fetch langsung dulu, jika gagal/CORS error baru beralih ke proxy
    try {
        const response = await fetch(url);
        if (!response.ok) throw new Error();
        return await response.json();
    } catch (e) {
        // Jika gagal karena CORS, gunakan proxy (Catatan: cors-anywhere membutuhkan aktivasi demo di websitenya)
        // Sebagai alternatif production, disarankan API dipanggil lewat backend server sendiri.
...
function handleRouting() {
    ...
    if (hash.startsWith('#detail/')) {
        ...
        // Menggunakan substring untuk mengambil seluruh sisa path setelah '#chapter/' 
        // agar slug yang mengandung '/' atau '%2F' tidak terpotong rusak
...
function renderError(retryCallback) {
    appRoot.innerHTML = `
        <div class="text-center py-16 px-4 bg-surface rounded-2xl border border-red-500/10">
            ...
            <p class="text-xs text-gray-500 mt-1 max-w-xs mx-auto">Masalah ini umumnya disebabkan oleh pembatasan keamanan CORS browser atau API limit.</p>
...
async function renderHome() {
    showLoader();
    try {
        const data = await safeFetch(`${BASE_API}/home`);
                        ...
                        <div class="flex flex-col justify-between min-w-0 flex-grow">
                            ...
                            <div class="grid grid-cols-2 gap-1.5 mt-2">
                                ${(item.chapters || []).slice(0, 2).map(c => {
                                    // Bersihkan slug untuk rute navigasi internal
                                    const numericSlug = c.slug ? c.slug.split('/').pop() : '';
                                    const displayTitle = c.slug ? c.slug.split('/').pop().split('.')[0].replace('-', ' ').toUpperCase() : 'Read';
...
async function renderDetailManga(slug) {
    ...
    try {
        ...
        if(!data || !data.success || !data.details) throw new Error();
                ...
                <div class="p-4 rounded-2xl bg-surface border border-gray-900 flex flex-col sm:flex-row gap-4">
                    ...
                    <div class="flex flex-col justify-between space-y-3 flex-grow text-center sm:text-left">
                        ...
                        <div class="flex flex-wrap justify-center sm:justify-start gap-1">
                            ${(manga.genres || []).map(g => `<span class="text-[10px] bg-dark px-2 py-0.5 rounded-md border border-gray-800 text-gray-400 font-medium">${g}</span>`).join('')}
                        </div>
                    </div>
                </div>
...
async function renderChapterReader(comicSlug, chapterSlug) {
    showLoader();
    try {
        // Lakukan fetch data menggunakan dynamic route encoder yang aman
        const data = await safeFetch(`${BASE_API}/chapter/${comicSlug}/${encodeURIComponent(chapterSlug)}`);
        if(!data || !data.success || !data.images) throw new Error();

        contextBar.textContent = `Ch. ${chapterSlug.split('.')[0].replace('chapter-', '')}`;

        // Mengurai navigasi URL dari payload API menjadi format Hash SPA internal
        const parseHashFromUrl = (url) => {
            if(!url) return null;
            const decodeUrl = decodeURIComponent(url);
            const parts = decodeUrl.replace(/\/$/, '').split('/');
            const ch = parts.pop();
            const cm = parts.pop();
        ...
        appRoot.innerHTML = `
            <div class="space-y-4 max-w-2xl mx-auto">
                <div class="flex items-center justify-between p-2 rounded-xl bg-surface border border-gray-900">
                    ...
                    <span class="text-xs text-gray-400 font-semibold truncate px-2 max-w-[180px]">${data.title}</span>
                </div>
